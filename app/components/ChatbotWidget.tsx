...
export default function ChatbotWidget() {
  ...
  useEffect(() => {
    if (typeof window !== 'undefined') {
      window.parent.postMessage({ type: isOpen ? 'openChat' : 'closeChat' }, '*');
    }
  }, [isOpen]);
  ...
  return (
    <div className="fixed bottom-6 right-6 transition-all duration-300 ease-in-out">
      {!isOpen ? (
        <Button
          onClick={() => setIsOpen(true)}
          className="w-14 h-14 sm:w-16 sm:h-16 rounded-full transition-all duration-300 p-0 relative overflow-hidden group"
          style={{
        ...
        <Card className="w-[350px] sm:w-[400px] h-[500px] flex flex-col border border-primary/10 overflow-hidden rounded-lg">
          <CardHeader className="flex flex-row items-center justify-between py-3 px-4" style={{
            background: 'linear-gradient(to right, #fcbc18 0%, #2b8894 100%)'
          }}>
            <div className="flex items-center space-x-3">
              <Bot size={24} className="text-white filter drop-shadow-md" />
              <CardTitle className="text-white text-lg font-bold tracking-wide" style={{
          ...
          <CardContent className="flex-grow overflow-y-auto p-4 space-y-4 bg-white">
            {messages.map((message, index) => (
              ...
              >
                <div
                  className={`p-3 rounded-lg max-w-[80%] shadow-md overflow-hidden ${
                    message.isUser
            ...
            {isLoading && (
              <div className="flex justify-start">
                <div className="bg-gradient-to-br from-[#2b8894] to-[#247885] text-white p-3 rounded-lg shadow-md">
                  <LoadingDots />
                </div>
              </div>
            )}
          ...
          <CardFooter className="border-t border-primary/10 p-3">
            <form onSubmit={handleSendMessage} className="flex w-full gap-2">
              ...
              <Button 
                type="submit" 
                size="icon" 
                className="rounded-full w-10 h-10 flex items-center justify-center transition-all duration-300 group hover:scale-105 shadow-lg"
                style={{
                  background: 'linear-gradient(135deg, #fcbc18 0%, #2b8894 100%)',
                }}
                disabled={isLoading}
